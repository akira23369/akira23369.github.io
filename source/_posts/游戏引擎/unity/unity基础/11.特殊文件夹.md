...
# **Resources 资源文件夹**
如果你希望在代码中动态加载资源（如预制体、图片、音频等），可以将这些资源放在 Resources 文件夹中，然后使用 Resources.Load 方法来加载。

**路径**：`Assets/Resources`
**特点**：
- **打包后只读 只能通过Resources相关API加载**，通过 `Resources.Load("资源名")` 动态加载资源（无需路径，忽略扩展名）。
...
- **需自己手动在Asset文件下创建Resources文件夹 名字一定不要错**
...
# **Plugins 插件文件夹**
存放第三方插件或原生代码（如 C++ 动态库、Java 类、.NET DLL 等）存放原生插件（如 .dll、.so、.a 文件）或平台特定代码。
不同平台的插件相关文件放在其中
...
**路径**：`Assets/Plugins`
**特点**：
 **需自己手动在Asset文件下创建Plugins文件夹 名字一定不要错**
**平台子文件夹**：
支持按平台拆分插件：可在 `Plugins` 下创建子文件夹（如 `Plugins/Android`、`Plugins/iOS`、`Plugins/Windows`），Unity 会根据目标平台自动加载对应文件夹的插件。
...
# **Editor 编辑器文件夹**
存放编辑器扩展脚本（如自定义 Inspector、编辑器窗口、打包工具）。
**路径**：`Assets/Editor`
**特点**：
 - **需自己手动在Asset文件下创建Editor文件夹 名字一定不要错**
 - 脚本**只在 Unity 编辑器中运行**，不会被打包到应用中。
- 可继承 `Editor` 类实现自定义功能。
- 可创建多个 `Editor` 文件夹（如 `Assets/Editor`、`Assets/Plugins/Editor`），方便按功能拆分编辑器工具。
